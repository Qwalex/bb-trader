...
```bash
cd apps/api && npm run test && npm run test:e2e
```
...
5. **Размер позиции:** в сигнале задаётся **сумма в USDT** (номинал по всем входам), по умолчанию **10 USDT**, если в тексте не указано иначе. Старый вариант «процент от депозита» поддерживается, если в JSON задан только `capitalPercent` (без суммы в USDT).
