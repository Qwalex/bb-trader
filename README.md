...
Полуавтоматизированная торговая система: Telegram-сигналы → OpenRouter (распознавание) → Bybit (ордера) → Postgres/Supabase (история, winrate, PnL). Веб-дашборд на Next.js 16.
...
Основная база хранится в Postgres volume `supabase-db-data`.

Визуальный просмотр БД доступен через Supabase Studio.
...
5. **Размер позиции:** в сигнале задаётся **сумма в USDT** (номинал по всем входам). Если в тексте не указано иначе, подставляется значение настройки **`DEFAULT_ORDER_USD`** (`/settings` или переменная окружения; при отсутствии — 10 USDT). Старый вариант «процент от депозита» поддерживается, если в JSON задан только `capitalPercent` (без суммы в USDT).
