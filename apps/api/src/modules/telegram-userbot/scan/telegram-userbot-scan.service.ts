  ...
  async getTodayMetrics() {
    ...
    return {
      ...
      recent: recent.map((row) => ({
        id: row.ingestId,
        chatId: row.chatId,
        messageId: row.ingest?.messageId ?? null,
        text: row.ingest?.text ?? null,
        aiRequest: row.aiRequest ?? row.ingest?.aiRequest ?? null,
        aiResponse: row.aiResponse ?? row.ingest?.aiResponse ?? null,
