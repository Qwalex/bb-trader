...
export class BybitService {
  ...
  private oppositeDirection(direction: 'long' | 'short'): 'long' | 'short' {
  ...
  private async isHedgeModeActiveForSymbol(
    client: RestClientV5,
    symbol: string,
  ): Promise<boolean> {
    try {
  ...
  private static isFinalOrderStatus(status: string | null | undefined): boolean {
  ...
  private async cancelTrackedSignalOrders(
    ...
    const orderIds = Array.from(
      new Set(
        signal.orders
          .filter((ord) => !BybitService.isFinalOrderStatus(ord.status))
    ...
    for (const orderId of orderIds) {
      ...
      if (!cancelled) {
        this.logger.warn(`Не удалось отменить tracked order ${symbol} ${orderId}`);
      }
    }
  ...
  private async countLiveTrackedOrders(
    ...
    const activeIds = new Set(
      (await this.getExchangeActiveOrders(client, symbol)).map((ord) => ord.orderId),
    );
    let live = 0;
    for (const orderId of trackedIds) {
      ...
      const status = await this.fetchOrderStatusFromExchange(
        client,
        signal.pair,
        orderId,
        dbOrder?.qty != null ? Number(dbOrder.qty) : undefined,
      );
      if (BybitService.isOpenOrderStatus(status)) {
        live += 1;
      }
    }
    return live;
  ...
  private async waitForSignalSideCleanup(
    ...
    while (Date.now() <= deadline) {
      const [activeOrders, positions] = await Promise.all([
        this.countLiveTrackedOrders(client, symbol, signal),
        this.getExchangePositions(client, symbol),
      ]);
      const sidePosition = BybitService.pickLiveExposurePositionForDirection(
        positions,
        direction,
      );
  ...
  private async closeSignalSideOnExchange(
    client: RestClientV5,
    symbol: string,
    direction: 'long' | 'short',
    signal: {
  ...
  > {
    const errors: string[] = [];
    let closedPositions = 0;
    const { qtyStep } = await this.getLotStep(client, symbol);
    ...
    try {
      const positions = await this.getExchangePositions(client, symbol);
      const p = BybitService.pickLiveExposurePositionForDirection(positions, direction);
      if (p) {
        const closeSide = direction === 'long' ? 'Sell' : 'Buy';
        const qty = this.formatQtyToStep(p.size, qtyStep);
        if (qty && parseFloat(qty) > 0) {
          const res = await client.submitOrder({
            category: 'linear',
            symbol,
  ...
  async placeSignalOrders(
    ...
    try {
      const hedgeModeActive = await this.isHedgeModeActiveForSymbol(client, symbol);
      const oppositeDirection = this.oppositeDirection(signal.direction);
      if (
        !hedgeModeActive &&
        ...
        void this.appLog.append('warn', 'bybit', 'placeSignalOrders: отказ (противоположная позиция/ордера на бирже)', {
          symbol,
          direction: signal.direction,
          oppositeDirection,
          hedgeModeActive,
  ...
  private async stepStopLossIfTpFilled(
    ...
    const posRow = BybitService.pickPositionRowForSignalDirection(
      posRows,
      direction,
    );
    if (!posRow) {
