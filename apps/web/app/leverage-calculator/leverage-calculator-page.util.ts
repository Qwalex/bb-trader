...
export function computeLeverageOutlook(params: {
  ...
  if (L > 0 && M > 0 && termM >= 1 && iMonth === null) {
    const minRatio = 1 / termM;
    if (M / L < minRatio - 1e-12) {
      const body =
        termM <= 1
          ? 'При сроке ровно один месяц платёж M не может быть ниже суммы долга L — иначе тело не закрывается даже без процентов.'
          : `Платёж M слишком мал относительно L и срока T месяцев: M/L < 1/T (доля «равномерного» тела L/${termM} в месяц). Для классического аннуитета с неотрицательной ставкой такая комбинация несовместима.`;
      warnings.push(`${body} Подразумеваемая годовая ставка по аннуитету из этой тройки не выводится.`);
    } else {
      warnings.push(
        'Параметры L, M, T не удалось согласовать с классическим ровным аннуитетом (численный разбор не сошёлся). Подразумеваемая годовая ставка не выведена — проверьте ввод и схему договора (возможно, это не аннуитет).',
      );
    }
  }
