...
- `SEC-013` (High), `apps/web/lib/api.ts` + browser-side API callers, production httpOnly auth cookie was not available to client-side `fetch`, causing API calls to be sent without Bearer and fail as 401; mitigated by same-origin Next BFF proxy that reads `sb_auth` server-side and forwards Authorization to API, status `mitigated`, linked task `AUD-051`.
- `SEC-014` (High), `apps/api/src/modules/auth/auth.service.ts` + `apps/api/prisma/schema.prisma`, password reset flow leaked account availability through request endpoint errors and lacked brute-force limits on code confirmation; mitigated by constant success response on reset request, per-code attempts + temporary lockout, and consume-all active codes on successful reset, status `mitigated`, linked task `AUD-054`.
- `SEC-015` (Low), `apps/api/src/modules/bybit/instrument/bybit-client.service.ts`, один глобальный private Bybit WebSocket подписывался на один набор API-ключей при нескольких кабинетах с разными ключами — события не соответствовали всем счетам; mitigated по умолчанию: при `prisma.cabinet.count() > 1` и настройке `BYBIT_WS_MULTI_CABINET=auto` (дефолт в `ENV_FALLBACK`) WS не поднимается, опрос остаётся через очередь; принудительно старое поведение — `BYBIT_WS_MULTI_CABINET=force`, status `mitigated`, linked task `AUD-059`.
