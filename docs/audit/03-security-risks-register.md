...
- `SEC-004` (Medium), `apps/api/src/modules/bybit/bybit.service.ts`, oversized orchestration service (~5k lines) increases chance of hidden logic regressions and weakens security review depth; mitigation in progress via wave-based decomposition (start: extracted JSON parsing helpers to `bybit-json.util.ts`), status `in_progress`, linked task `AUD-006`.
