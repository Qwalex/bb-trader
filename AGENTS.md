...
- Запуск Telegraf на кабинет (`deleteWebhook` + `launch`): **глобальная очередь** — одновременно не более одного launch на процесс API; между запусками — пауза `TELEGRAM_BOT_LAUNCH_STAGGER_MS` (по умолчанию 2000 мс, `0` — выкл.). Таймаут одной попытки: `TELEGRAM_BOT_LAUNCH_TIMEOUT_MS` (по умолчанию 60000 мс, диапазон 5000–180000); при срабатывании таймаута фоновый `launch()` не регистрирует бота (нет пары ERROR + «started» в логах); при медленной сети к Bot API имеет смысл поднять таймаут на Railway.
