...
- Запуск Telegraf на кабинет (`deleteWebhook` + `launch`): **глобальная очередь** — одновременно не более одного launch на процесс API; между запусками — пауза `TELEGRAM_BOT_LAUNCH_STAGGER_MS` (по умолчанию 2000 мс, `0` — выкл.). Таймаут **`deleteWebhook`**: `TELEGRAM_BOT_DELETE_WEBHOOK_TIMEOUT_MS` (по умолчанию 30000 мс, 5000–120000); таймаут **`bot.launch()` / getUpdates**: `TELEGRAM_BOT_LAUNCH_TIMEOUT_MS` (по умолчанию 60000 мс, 5000–180000). В логах — фазы и `correlationId`/`syncId`; при сбое — повторный `syncBotsWithCabinetTokens` с backoff 5/15/30 с; при медленной сети к Bot API имеет смысл поднять таймауты на Railway.
