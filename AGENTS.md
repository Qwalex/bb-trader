...
- **Userbot (API):** фасад `telegram-userbot.service.ts` — координация; часть ingest (в т.ч. `listIngestLinkCandidates`, `rereadIngestMessage`, `rereadAllIngestMessages`) — в **`ingest/telegram-userbot-ingest-pipeline.service.ts`** (см. AUD-048, `docs/telegram-userbot-decomposition-plan.md`). При старте API при `TELEGRAM_USERBOT_ENABLED` и сохранённой глобальной сессии восстанавливается MTProto для кабинета **владельца сессии** (`TELEGRAM_USERBOT_SESSION_OWNER_USER_ID` в `Setting`, выставляется при QR/ручном connect); если ключа ещё нет (старые инсталляции) — fallback на кабинет по умолчанию / первый с `ownerUserId`. Выключение автоподъёма: `TELEGRAM_USERBOT_SKIP_STARTUP_RESTORE=true` (см. AUD-069). Несколько реплик API с одной и той же MTProto-сессией могут гоняться за соединением и провоцировать повторный вход.
