...
- У каждого кабинета свой `TELEGRAM_BOT_TOKEN` в настройках (или env с разрешением по кабинету): long polling поднимает `syncBotsWithCabinetTokens`; **исходящие** уведомления (userbot, диагностика, сделки и т.д.) идут через Telegraf кабинета в процессе либо через временный клиент по токену этого кабинета — без подмены бота другого кабинета и с доставки с реплики API, где нет активного `getUpdates` (входящие апдейты по-прежнему только там, где успешно `launch`).
