...
- **QPulse (AUD-171):** отдельный monorepo `c:\Users\qwazi\Рабочий стол\QPulse` (mobile + admin + API, pnpm). Полная карта связи: **`docs/qpulse-ecosystem.md`**. Admin seed: `admin@qpulse.app` / `admin123` (после `prisma db seed` или `RUN_SEED=true` на Railway). В signalsBot: `/my-group` — publish-группы + **`linkedToApp`**; sync → `POST/PATCH /integrations/signals` с `X-API-Key`. Ключи кабинета: `QPULSE_SYNC_ENABLED`, `QPULSE_API_URL`, `QPULSE_API_KEY` (= QPulse `INTEGRATIONS_API_KEY`). Lifecycle PATCH только при `SignalExternalSync`. Content-кабинеты QPulse не используют (AUD-195).
- **Информационные кабинеты (AUD-195):** `Cabinet.purpose` = `trading` | `content`. Content — без Bybit/bot warnings, ingest не торгует; сбор в редактор по `CONTENT_COLLECT_KINDS` (analysis/content/news/other). Редактор `/content-editor`: фильтры, AI-пресеты (`ContentGenerationPreset`), cron `CONTENT_GENERATION_CRON` / `CONTENT_GENERATION_ENABLED`.
...
- **Worker split (AUD-197, окружение `cabinets`):** три Railway-сервиса из одного repo/образа API — **Api** (`API_PROCESS_ROLE=api`, `railpack.json`, `railway.api.toml`), **Worker-UB** (`worker-userbot`, `railpack.worker-userbot.json`, `railway.worker-userbot.toml` — MTProto userbot, ingest, VK, transcript LLM), **Worker-Bybit** (`worker-bybit`, `railpack.worker-bybit.json`, `railway.worker-bybit.toml` — worker queue, Bybit poll/WS/REST). Общий `DATABASE_URL`. Межсервисно: `WORKER_INTERNAL_TOKEN`, `WORKER_BYBIT_INTERNAL_URL`, `WORKER_UB_INTERNAL_URL`, `API_INTERNAL_URL` (Railway private `*.railway.internal:PORT`). На Api: `TELEGRAM_USERBOT_SKIP_STARTUP_RESTORE=true`, `WORKER_QUEUE_ENABLED=false`. **Ровно 1 реплика Worker-UB** (MTProto). Rollback: redeploy Api или `API_PROCESS_ROLE=all` на одном Api.
