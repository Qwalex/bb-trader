...
- Bybit при нескольких кабинетах: private WS по умолчанию не поднимается (`BYBIT_WS_MULTI_CABINET=auto`, см. `BybitClientService`); старт WS — после `OnApplicationBootstrap`, при отсутствии ключей на первом проходе — одна повторная попытка через 10 с (ключи без контекста кабинета читаются из глобального `Setting` и env). Каждый REST к Bybit на кабинет идёт через `BybitRateLimitService.runBybitCall` — одна очередь на кабинет, интервал `BYBIT_ACCOUNT_REQUEST_INTERVAL_MS` между запросами, backoff/retry при rate limit (`retCode=10006` и аналоги); `BYBIT_ACCOUNT_MAX_CONCURRENCY>1` в рантайме игнорируется (ключ зарезервирован); placement/stale in-memory ключи включают `cabinetId`.
